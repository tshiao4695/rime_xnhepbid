...
13.  标点符号的使用与一般的拼音输入法不同。在于常用标点符号是使用双拼输入的。这么做的原因是语音思维导致的，只需要熟记双拼规则及其键位就不需要26字母以外的按键，最主要的作用就是中英文切换的问题。英文标点可以直接在中文模式下输入。解决一些场景的应用问题。这种编码输入的思维主要是在扩充图形符号的范围。希腊字母和emoji那些东西也都是这个办法。使用这个方法也可以解决一个中文编程的烂梗——中文输入没有英文输入快的问题。常用的中文编程关键字直接把优先级调到第一或者映射出一个专门选择编程用语的按键即可(rime现在我不知道怎么实现，但是搜狗可以指定候选框中的第几位位用户自定义短语，由此实现特点候选位为某种特定符号的唯一身份码)(同理，假设候选位3为编程用语，则可以设定候选位2为数理逻辑用语，同理4、5、6等亦可)
名称        符号        编码
句号        。        ju
逗号        ，        dz
分号        ；        ff
